...
class MemoryManager(BaseAgent):
    ...
    async def get_calibration_data(self, db: AsyncSession) -> dict:
        """Compute calibration stats from closed trades.

        Uses a single batch query for lessons instead of N+1 per trade.
        """
        ...
        # Batch-load all lessons for these trades in one query
        trade_ids = [t.id for t in closed_trades]
        lessons_q = await db.execute(
            select(LessonLearned.trade_id, LessonLearned.category)
            .where(LessonLearned.trade_id.in_(trade_ids))
        )
        trade_category_map: dict[int, str] = {
            row.trade_id: row.category
        ...
        for t in closed_trades:
            ...
            cat = trade_category_map.get(t.id, "general")
    ...
    async def store_manual_lesson(
        ...
        db.add(lesson)
        self.log.info("manual_lesson_stored", category=entry.category)
